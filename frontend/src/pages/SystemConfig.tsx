...
import { Card, Form, InputNumber, Button, message, Descriptions, Spin } from 'antd'
...
export default function SystemConfig() {
  ...
  const fetchConfig = async () => {
    ...
    try {
      ...
      setConfig(data)
      form.setFieldsValue({
        maxBorrowCount: parseInt(data.max_borrow_count || '5'),
        borrowDays: parseInt(data.borrow_days || '30'),
        overdueFinePerDay: parseFloat(data.overdue_fine_per_day || '0.1')
      })
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      ...
      await Promise.all([
        configAPI.updateConfig('max_borrow_count', String(values.maxBorrowCount)),
        configAPI.updateConfig('borrow_days', String(values.borrowDays)),
        configAPI.updateConfig('overdue_fine_per_day', String(values.overdueFinePerDay))
      ])
